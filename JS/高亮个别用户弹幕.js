    ...
    if (location.origin == 'https://live.bilibili.com') {
        ...
        async function launch() {
            ...
            function handleUserEnter(uid, uname) {
                ...
                console.info(`【HighLight】(标记用户): ${uname} has enter this live room - ${new Date()}`)
            ...
            unsafeWindow.bliveproxy.addCommandHandler('DANMU_MSG', command => {
                //console.log(command);
                const userId = command.info[2][0], uname = command.info[2][1]
                var flag = highlightUsers.includes(userId);
                //console.debug(`user send danmu: ${userId}`)
                if (!(flag || command.info[2][2] || userId == 2061407)) return
                ...
                const pbcl = ["[", "]", "\\", "/", "call", "Call"]
                for (let pbc of pbcl) {
                    if (command.info[1].indexOf(pbc) != -1) {
                        //console.info(`【HighLight】Detect ${pbc}`)
                        return
                    }
                }
                ...
                console.info(`【HighLight】(${typ})${uname}:${command.info[1]} - ${new Date()}`)
                if (typ != "房管") {
                ...
                if (settings.playAudioDanmu && flag) audio.danmu.play()
            })
            //自定义结束
