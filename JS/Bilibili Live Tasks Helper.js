        ...
        getRoomidUidList() {
            ...
            if (biliStore.filteredFansMedals) {
                let idList1 = biliStore.filteredFansMedals.filter(
                    (medal) => medal.medal.today_feed < 200 && (this.config.includeHighLevelMedals ? true : medal.medal.level < 20) && (this.medalTasksConfig.isWhiteList ? this.medalTasksConfig.roomidList.includes(medal.room_info.room_id) : !this.medalTasksConfig.roomidList.includes(medal.room_info.room_id))
                ).map((medal) => [medal.room_info.room_id, medal.medal.target_id])
                this.logger.log(`点赞直播列表${idList1}(${idList1.length})`)
                return idList1;
            } else {
                this.status = "error";
                return null;
